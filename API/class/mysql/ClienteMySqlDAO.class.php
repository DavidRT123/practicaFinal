...
class ClienteMySqlDAO implements ClienteDAO{
	...
	public function insert($cliente){
		$sql = 'INSERT INTO cliente (nombres, ciudad, sexo, telefono, fecha_nacimiento, direccion, provincia, fecha_alta) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
		$sqlQuery = new SqlQuery($sql);
		...
		$sqlQuery->set($cliente->provincia);
		$sqlQuery->set($cliente->fechaAlta);
	...
	public function update($cliente){
		$sql = 'UPDATE cliente SET nombres = ?, ciudad = ?, sexo = ?, telefono = ?, fecha_nacimiento = ?, direccion = ?, provincia = ?, fecha_alta = ? WHERE id = ?';
		$sqlQuery = new SqlQuery($sql);
	...
	public function queryByProvincia($value){
		$sql = 'SELECT * FROM cliente WHERE provincia = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->getList($sqlQuery);
	}
	...
	public function queryByFechaAlta($value){
		$sql = 'SELECT * FROM cliente WHERE fecha_alta = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->getList($sqlQuery);
	}
	...
	public function deleteByProvincia($value){
		$sql = 'DELETE FROM cliente WHERE provincia = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->executeUpdate($sqlQuery);
	}
	...
	public function deleteByFechaAlta($value){
		$sql = 'DELETE FROM cliente WHERE fecha_alta = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->executeUpdate($sqlQuery);
	}
	...
	protected function readRow($row){
		...
		$cliente->fechaAlta = $row['fecha_alta'];
